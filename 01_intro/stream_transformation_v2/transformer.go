...
func main() {
	...
	collection, err := connectMongo(ctx)
	if err != nil {
		log.Fatalf("error in mongodb connection: %v", err)
	}
	...
	for {
		...
		_, err = collection.InsertOne(ctx, outputEvent)
		if err != nil {
			log.Println("Kafka output insertion to db error: ", err)
		} else {
			log.Println("MONGODB: event persisted")
		}
	}
}
...
func connectMongo(ctx context.Context) (*mongo.Collection, error) {
	...
	collection := client.Database("event_store").Collection("processed_user_events")
